...
interface Props {
  ...
  lisaaUusi : (otsikko: string,  kirjoittaja: string, sisalto: string) => void
}
...
const Muokkaus: React.FC<Props> = (props : Props) : React.ReactElement => {
  ...
  const tallenna = (e: React.FormEvent) : void => {
    ...
    if(formRef.current.kirjoittaja.value){
      kirjoittaja = formRef.current.kirjoittaja.value;
    }
  ...
  return <Dialog
            ...
            onClose={peruuta}
          >
          <DialogTitle>Lisää uusi keskustelu</DialogTitle>
